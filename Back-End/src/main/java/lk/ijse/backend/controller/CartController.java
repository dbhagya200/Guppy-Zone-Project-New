...
public class CartController {
    ...
    public ResponseEntity<ResponseDTO> getCart(@RequestHeader("Authorization") String token) {
        try {
            ...
            List<CartDTO> cartItems = cartService.getCartItemsByUser(email);
            System.out.println("Found " + cartItems.size() + " cart items"); // Debug log

            return ResponseEntity.ok()
                    .body(new ResponseDTO(VarList.OK, "Success", cartItems));
        } catch (Exception e) {
    ...
    @PreAuthorize("hasAnyAuthority('SELLER','BUYER')")
    public ResponseEntity<ResponseDTO> deleteCart(@RequestParam ("id") int id,
                                                  @RequestHeader("Authorization") String token) {
        cartService.removeFromCart(id);
        return ResponseEntity.ok()
                .body(new ResponseDTO(VarList.OK, "Cart deleted successfully", null));
    }
    ...
    public ResponseEntity<ResponseDTO> updateCart(@PathVariable int id,
        ...
        return ResponseEntity.ok().body(new ResponseDTO(VarList.OK, "Cart updated successfully", cartDTO));
    }
